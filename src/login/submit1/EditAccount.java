...
public class EditAccount  {
	...
	public void edituser() {
		try {
			...
			String pathUrl = "http://" + LoginActivity.Homeip + "/account/changepassword/";
			// 使用POST的方式建立連線
	...
	public void editnotify(String method) {
		try {
			...
			String pathUrl1 = "http://" + LoginActivity.Homeip + "/account/setnotification/";
	...
	public String getnotify() {
		...
		try {
			String pathUrl1 = "http://" + LoginActivity.Homeip + "/account/getnotification/";
			HttpRequest request1 = HttpRequest.get(pathUrl1);
			body = request1.header("cookie", Login.latest_cookie).body();
			boolean result=att_parameter.chechsuccess(body);
			if(result){
