...
public class retrieve {
	...
	public String devicepath = Environment.getExternalStorageDirectory().toString() + File.separator + "KM" + "/";
	...
	public String[] retrieve_req(String token, String mod) {
		...
		try {
			...
			String pathUrl = "http://" + logininput.Homeip + "/cms/retrieve/";
			...
			if (HttpURLConnection.HTTP_OK == responseCode) {
				...
				boolean result = att_parameter.chechsuccess(rreqreadLine);
		...
		catch (HttpRequestException nullpoint) // 判斷帳號密碼錯誤
		{
			reretrieve[0] = "server error";
			//
			reretrieve[3] = "true";
	...
	public String[] saveBinaryFile(String token, int i) throws IOException {
		...
		try {
			...
			String pathUrl = "http://" + logininput.Homeip + "/cms/retrieve_file/";
			HttpRequest request = HttpRequest.post(pathUrl);
			/*
			...
			if(request.code()==200){
				...
				if(filename!=null){
					...
					checktype = att_parameter.checktype(filename);
					
					if (checktype[att_parameter.video]||checktype[att_parameter.photo]) {
							filename = token+"_"+filename.replace(".", "-_" + String.valueOf(i) + ".");					
					}
					File output1 = new File(devicepath+filename);
					if(!output1.exists()){
				...
				refile[2]="true";
				File output = new File(devicepath+"error.html");
	...
	public String[] saveBinaryFile_for_d2d(String token,int i,ContentResolver content) throws IOException {
		...
		Long actual_length;
		int downLength = 0;
		//int record_number=314572;//每0.3M
		...
		try {
			 ...
			 */
				if(request.code()==200){
					...
					boolean[] checktype = new boolean[att_parameter.filetype];
					checktype = att_parameter.checktype(savefilename);
					
					if (checktype[att_parameter.video]||checktype[att_parameter.photo]) {
						savefilename = token+"_"+savefilename.replace(".", "-_" + String.valueOf(i) + ".");										
					}
					File output1 = new File(devicepath+savefilename);
	...
	public String conversation(String localtoken, String mod, int urgent) {
		String reretrieve = new String();
		token = localtoken;
		try {
			String pathUrl = null;
			if (mod.equals("request")) {
				pathUrl = "http://" + logininput.Homeip + "/cms/request/";
			} else if (mod.equals("reply")) {
				pathUrl = "http://" + logininput.Homeip + "/cms/reply/";
			}
			HttpRequest request = HttpRequest.post(pathUrl);
			// 設定cookie並傳入token
			int responseCode = request.header("cookie", Login.latest_cookie).send("token=" + token + "&urgent=" + urgent).code();
			// 如果response回覆是OK
			if (HttpURLConnection.HTTP_OK == responseCode) {
				// 這邊就會取得跟此token有關的主旨、內容、檔案的id及檔案名稱
				rreqreadLine = request.body();
				System.out.println("rreqreadLine=" + rreqreadLine);
				boolean result = att_parameter.chechsuccess(rreqreadLine);
				// 把篩選完的rreqreadLine放入reretrieve[0]
				if (result) // check ret=0
				{
	...
	public String check_file(String localtoken) {
		...
		token = localtoken;
		try {
			String pathUrl = "http://" + logininput.Homeip + "/cms/check_file/?token="+token;
