...
public class regFromServer {
	...
	try {
	    ...
	    String pathUrl = "http://" +  logininput.Homeip + "/account/register/";
	...
	try {
	    ...
	    // 設定mail內容
	    String message = "http://"+LoginActivity.Homeip+"/account/validate/?" + validate[1];
	    // 設定mail的收件者
	    String to_email = validate[3];
	    String requestString = "subject=" + subject + "&message=" + message + "&to_email=" + to_email;
	    String pathUrl = "http://"+LoginActivity.Homeip+":8000/cms/send_email/";
	    HttpRequest request = HttpRequest.post(pathUrl);
	...
	try {
	    ...
	    String pathUrl = "http://" +  LoginActivity.Homeip + "/account/validate/";
