...
public class messageProcess {
	...
	private NotificationManager gNotMgr = null;
	   ...
	   public  void checkwlan(ContentResolver content, String aliveIp) {
			...
			boolean[] checktype = new boolean[att_parameter.msgtype];
			checktype = att_parameter.checkCR(aliveIp);
			if (checktype[att_parameter.content] & checktype[att_parameter.reply]) {
			...
			}else if(checktype[att_parameter.content] & checktype[att_parameter.d2d]) {
			...
			else if(checktype[att_parameter.reply] & checktype[att_parameter.d2d]) {
				
			    S_rep = aliveIp.substring(aliveIp.indexOf("&reply") + 6,aliveIp.indexOf("&d2d") );
			    S_d2d = aliveIp.substring(aliveIp.indexOf("&d2d") + 4, aliveIp.length());
				}
			else if(checktype[att_parameter.content] & checktype[att_parameter.reply] & checktype[att_parameter.d2d]) {
			    S_ret = aliveIp.substring(aliveIp.indexOf("&content") + 8, aliveIp.indexOf("&reply"));
			    S_rep = aliveIp.substring(aliveIp.indexOf("&reply") + 6,aliveIp.indexOf("&d2d") );
			    S_d2d = aliveIp.substring(aliveIp.indexOf("&d2d") + 4, aliveIp.length());
			}
			else if (checktype[att_parameter.content]) {
			    S_ret = aliveIp.substring(aliveIp.indexOf("&content") + 8, aliveIp.length());
			} else if (checktype[att_parameter.reply]) {
			    S_rep = aliveIp.substring(aliveIp.indexOf("&reply") + 6, aliveIp.length());
			}else if (checktype[att_parameter.d2d]) {
			    S_d2d = aliveIp.substring(aliveIp.indexOf("&d2d") + 4, aliveIp.length());
			}
